 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      // angel
      fg[0] += c_espi * CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
      // target V
      fg[0] += c_vel * CppAD::pow(vars[v_start + i] - ref_v, 2);
    }

    // Minimize actuators
    for (int i = 0; i < N - 1; i++) {
      fg[0] += c_act_steer * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += c_act_vel * CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the gap between sequential actuations
    for (int i = 0; i < N - 2; i++) {
      fg[0] += c_delta_steer * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += c_delta_vel * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  std::vector<double> result = {solution.x[delta_start], solution.x[a_start], (double)N};
